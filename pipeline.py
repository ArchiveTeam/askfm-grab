...
LUA_SHA1 = get_hash(os.path.join(CWD, 'askfm.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'user': # done
                wget_args.extend(['--warc-header', 'askfm-user: '+item_value])
                wget_args.append('https://ask.fm/'+item_value)
            #elif item_type == 'tag':
            #    wget_args.extend(['--warc-header', 'askfm-tag: '+item_value])
            #    wget_args.append('https://ask.fm/tags/'+item_value)
            elif item_type == 'asset': # done
                wget_args.extend(['--warc-header', 'askfm-user: '+item_value])
                wget_args.append('https://'+item_value)
            elif item_type == 'user-page': # done
                user, path, page = item_value.split(':')
                if len(path) > 0:
                    path = '/' + path
                wget_args.extend(['--warc-header', 'askfm-user-page: '+item_value])
                wget_args.append('https://ask.fm/{}{}?page={}'.format(user, path, page))
            elif item_type == 'answer': # done
                user, answer = item_value.split(':')
                wget_args.extend(['--warc-header', 'askfm-user-answer: '+item_value])
                wget_args.extend(['--warc-header', 'askfm-answer: '+answer])
                wget_args.append('https://ask.fm/{}/answers/{}'.format(user, answer))
            elif item_type == 'thread': # done
                user, thread = item_value.split(':')
                wget_args.extend(['--warc-header', 'askfm-user-thread: '+item_value])
                wget_args.extend(['--warc-header', 'askfm-thread: '+thread])
                wget_args.append('https://askfm.site/{}/threads/{}'.format(user, thread))
            elif item_type == 'photopoll': # done
                user, thread = item_value.split(':')
                wget_args.extend(['--warc-header', 'askfm-user-photopoll: '+item_value])
                wget_args.extend(['--warc-header', 'askfm-photopoll: '+thread])
                wget_args.append('https://ask.fm/{}/photopolls/{}'.format(user, thread))
            elif item_type == 'question': # done
                user, question = item_value.split(':')
                wget_args.extend(['--warc-header', 'askfm-country-question: '+item_value])
                wget_args.extend(['--warc-header', 'askfm-question: '+question])
                wget_args.append('https://ask.fm/countries/{}/shoutouts/{}'.format(user, question))
            else:
